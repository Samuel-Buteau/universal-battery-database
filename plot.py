...
def generate_plot_options(mode: str, typ: str, target: str) -> dict:
    ...
    if target == "generic_vs_capacity":
        ...
        raise Exception(target_error(target, mode, "generate_options"))
...
def get_y_quantity(mode: str) -> str:
    ...
    """
    if mode == "cc" or mode == "q":
        y_quantity = "voltage"
    elif mode == "cv":
        y_quantity = "current"
    else:
        raise Exception(target_error("", mode, "get_y_quantity"))
    return y_quantity
...
def get_generic_map(source, target: str, mode: str) -> dict:
    ...
    if target == "generic_vs_cycle":
        ...
        raise Exception(target_error(target, mode, "get_generic_map"))
    return generic_map
...
def data_engine(
    ...
    if source == "model":
        ...
        raise Exception(target_error(target, mode, "data_engine"))
...
def compute_target(
    ...
    if target == "generic_vs_capacity":
        ...
        q = degradation_model.test_all_voltages(
        ...
        cap = tf.reshape(
            q[pred_capacity_label], shape = [max_cyc_n, -1],
        )
        ...
        cap = tf.reshape(q, shape = [max_cyc_n, -1])
        ...
        q = degradation_model.test_single_voltage(
        ...
        if mode == "cc":
            pred_cap = tf.reshape(
                q[Key.Pred.I_CC], shape = [-1],
            ).numpy()
        elif mode == "cv":
            pred_cap = q[Key.Pred.I_CV].numpy()[:, -1]
        else:
            raise Exception(target_error(target, mode, "compute_target"))
...
def target_error(target: str, mode: str, function_name: str) -> str:
    return (
