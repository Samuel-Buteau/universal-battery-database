...
def debug_crop_cycles(crop):
    ...
    CyclingFile.objects.all().update(import_time=datetime.datetime(1970, 1, 1))
    CyclingFile.objects.all().update(process_step_time=datetime.datetime(1970, 1, 1))
    CyclingFile.objects.all().update(process_cycle_time=datetime.datetime(1970, 1, 1))
